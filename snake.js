...
const game = {
    ...
    getStartSnakeBody() {
        return [
            {
                x: Math.ceil(this.config.getColsCount() / 2),
                y: Math.ceil(this.config.getRowsCount() / 2),
            }
        ];
    },
    ...
    getRandomFreeCoordinates() {
        ...
        while (true) {
            const rndPoint = {
                x: Math.ceil(Math.random() * this.config.getColsCount()),
                y: Math.ceil(Math.random() * this.config.getRowsCount()),
            }
    ...
    snakeMakeStep() {
        ...
        this.snake.lastStepDirection = this.snake.direction;
        snakeBody.unshift(nextHeadPoint);
        snakeBody.pop();
    },

    countFood() {
        const count = document.getElementById('counter');
        count.innerHTML = `Счет: ${snake.getBody().length - 1}`;
    },
};
